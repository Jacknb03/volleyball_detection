...
| 模式 | YAML | 关键字段 |
|------|------|----------|
| video | `config/ball_detector_params_video.yaml` | `position.mode: bbox` |
| realsense | `config/ball_detector_params_realsense.yaml` | `position.mode: depth` + RealSense 话题 |
...
自动加载 `ball_detector_params_realsense.yaml`：
...
| 文件 | 作用 |
|------|------|
| `config/ball_detector_params_video.yaml` | 视频 bbox 模式参数 |
| `config/ball_detector_params_realsense.yaml` | RealSense depth 模式参数 |
| `launch/yolo.launch.py` | **统一入口**（pipeline_mode） |
